...
void* JSDelta::creator() { return new JSDelta(); }
...
MStatus JSDelta::initialize() {
	...
	//aAutoKickGPU = fnNumAttr.create("autoKickGPU", "autoKickGPU", MFnNumericData::kBoolean, true);
...
MStatus JSDelta::deform(MDataBlock& data, MItGeometry& iterGeo, 
	...
	if (smoothIterations != previousNumIterations_)
	{
		...
		//// Using static attr as buffer for GPU because not pulling from deltas_...
		//MDataHandle hDeltasBuffer = data.outputValue(aDeltasBuffer);
...
cl_int JSDeltaGPU::initializeKernel(MDataBlock &data, const MPlug &plug, const unsigned int numElements)
{
	...
	//// Fill deltas from datablock // Filling from buffer attr on datablock to allow change to smooth iterations // or not
